...
#
# $Id: schdev.pl,v 1.4 2006/11/10 21:50:55 mroha Exp $
...
if ($opt_debug) {
  foreach my $macro (sort keys %dev_record) {
    $MAINLOG->infod("Macro:$macro Devices:$dev_record{$macro}{'DEVICES'} Unique Devices:$dev_record{$macro}{'HIERDEV'} Unique Devices Not In STD Cells:$dev_record{$macro}{'NONSTDHIERDEV'} Flat Devices:$dev_record{$macro}{'FLATDEV'} Flat Devices Not in STD Cells:$dev_record{$macro}{'NONSTDFLATDEV'} ");
    if (defined $dev_record{$macro}{'INSTMASTERS'}) {
      foreach my $instmaster (sort keys %{ $dev_record{$macro}{'INSTMASTERS'} }) {
	$MAINLOG->infod("    Instance $instmaster ($dev_record{$macro}{'INSTMASTERS'}{$instmaster}) ($dev_record{$instmaster}{'HIERDEV'})  ($dev_record{$instmaster}{'NONSTDHIERDEV'}) ($dev_record{$instmaster}{'FLATDEV'}) ($dev_record{$instmaster}{'NONSTDFLATDEV'})");
      }
    }
  }
}
...
print DEVREPORT "# (<instance count>) <cell> (<hierarchy level>) (<devices only in current hierarchy>) (<unique devices in hierarchy>) (<non stdcell unique devices in hierarchy>) (<flat devices in hierarchy>) (<non stdcell flat devices in hierarchy>)\n\n";
unless ($opt_nocsvheader) {
  ...
  @csvfields = (@csvfields, "non stdcell unique devices in hierarchy", "flat devices in hierarchy", "non stdcell flat devices in hierarchy");
  if (exists $external_csv{$cell_lc}) {
...
sub SetHierUniqueDevices {
  ...
  unless (defined $$dev_record_ref{$cell_lc}{'HIERDEV'}) {
	...
	$$dev_record_ref{$cell_lc}{'NONSTDHIERDEV'}+= $$dev_record_ref{$macro}{'NONSTDHIERDEV'};
      }
    }
    $$dev_record_ref{$cell_lc}{'HIERDEV'}+= $$dev_record_ref{$cell_lc}{'DEVICES'};
    if ($cell_lc =~ /^(vm0|v00)\S+$/) {
      $$dev_record_ref{$cell_lc}{'NONSTDHIERDEV'}+= 0;
    } else {
      $$dev_record_ref{$cell_lc}{'NONSTDHIERDEV'}+= $$dev_record_ref{$cell_lc}{'DEVICES'};
    }
  }				   
}
...
sub SetHierFlatDevices {
  ...
  unless (defined $$dev_record_ref{$cell_lc}{'FLATDEV'}) {
	...
	$$dev_record_ref{$cell_lc}{'NONSTDFLATDEV'}+= ($$dev_record_ref{$macro}{'NONSTDFLATDEV'}*$$dev_record_ref{$cell_lc}{'INSTMASTERS'}{$macro});
      }
    }
    $$dev_record_ref{$cell_lc}{'FLATDEV'}+= $$dev_record_ref{$cell_lc}{'DEVICES'};
    if ($cell_lc =~ /^(vm0|v00)\S+$/) {
      $$dev_record_ref{$cell_lc}{'NONSTDFLATDEV'}+= 0;
    } else {
      $$dev_record_ref{$cell_lc}{'NONSTDFLATDEV'}+= $$dev_record_ref{$cell_lc}{'DEVICES'};
    }
  }				   
}
...
sub GenerateDevReport {
  ...
  my $line = sprintf "%-55s %4s %8s %10s %10s %10s %10s", "${spacer} (${instmasters}) $cell", "(${level})", "($$dev_record_ref{$cell}{'DEVICES'})", "($$dev_record_ref{$cell}{'HIERDEV'})", "($$dev_record_ref{$cell}{'NONSTDHIERDEV'})", "($$dev_record_ref{$cell}{'FLATDEV'})", "($$dev_record_ref{$cell}{'NONSTDFLATDEV'})";
  my $csv_record = join (',', $instmasters, "${spacer}${cell}", $level, $$dev_record_ref{$cell}{'DEVICES'}, $$dev_record_ref{$cell}{'HIERDEV'}, $$dev_record_ref{$cell}{'NONSTDHIERDEV'}, $$dev_record_ref{$cell}{'FLATDEV'}, $$dev_record_ref{$cell}{'NONSTDFLATDEV'});
