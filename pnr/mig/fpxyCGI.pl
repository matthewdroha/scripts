...
#
# $Id: fpxyCGI.pl,v 1.2 2005/10/10 16:00:09 mroha Exp $
...
Generate html from migstat.pl fp XY output
...
my $ps2_fpxy_csv = "${www_data}/migstat.pshift2_fpxy.csv";
unless (-e $ps2_fpxy_csv) {
  die "Could not find status table in web area: $ps2_fpxy_csv";
}

my @csv_data;
open (STATUS, $ps2_fpxy_csv) or die "Could not open status table in web area: $ps2_fpxy_csv";
while (<STATUS>) {
  chomp;
  push (@csv_data, $_);
} 
...
print "<html>\n";
print "<head>\n<title>Pshift2 vs Section Floorplan Fub Sizing</title>\n";
print "</head>\n";
print "<body>\n";
print "<font face=verdana size=4>\n";
print "<b>Pshift2 vs Section Floorplan Fub Sizing</b><br><br>\n";
