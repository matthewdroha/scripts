...
#
# $Id: psxyCGI.pl,v 1.2 2005/10/10 16:00:10 mroha Exp $
...
Generate html from migstat.pl pshift XY output
...
my $ps2_psxy_csv = "${www_data}/migstat.pshift2_psxy.csv";
unless (-e $ps2_psxy_csv) {
  die "Could not find status table in web area: $ps2_psxy_csv";
}

my @csv_data;
open (STATUS, $ps2_psxy_csv) or die "Could not open status table in web area: $ps2_psxy_csv";
while (<STATUS>) {
  chomp;
  push (@csv_data, $_);
} 
...
print "<html>\n";
print "<head>\n<title>Pshift2 vs Pshift1 Fub Sizing</title>\n";
print "</head>\n";
print "<body>\n";
print "<font face=verdana size=4>\n";
print "<b>Pshift2 vs Pshift1 Fub Sizing</b><br><br>\n";
