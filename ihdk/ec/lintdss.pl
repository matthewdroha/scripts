...
#
# $Id: lintdss.pl,v 1.1 2015/12/23 02:54:40 mroha Exp $
...
while (<$incsvfh>) {
  ...
  elsif (scalar @record != 53) {
    #if (/\"/) {
    #  $log->infod("Found embedded quotes: Line $.->($_)");
    #} else {
    $log->infow("Field Count Problem: Line $.->($_)");
    next;
  } else {
    my $foundBadDate = 0;
    foreach my $field (@dateFields) {
      if (($record[${field}]) and ($record[${field}] !~ /^\d{4}\-\d{2}\-\d{2} \d{2}\:\d{2}\:\d{2}/)) {
	$log->infow("Date format is incorrect: Line $.->($_)");
	$foundBadDate = 1;
	last;
      }
      if ($foundBadDate) {next}
    }
    my $fieldNumber = 0;
