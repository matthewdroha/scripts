...
#
# $Id: lintcsq.pl,v 1.1 2015/12/23 02:54:40 mroha Exp $
...
while (<$incsvfh>) {
  ...
  elsif (scalar @record != 20) {
    $log->info("Field Count Problem: Line $.->($_)");
  } else {
...
#    }
    ...
    if ($record[11] eq 'UNKNOWN') {
      $log->info("Unknown OS Image: Line $.->($_)");
    } else {
      $writeRecord = 1;
    }
  }
