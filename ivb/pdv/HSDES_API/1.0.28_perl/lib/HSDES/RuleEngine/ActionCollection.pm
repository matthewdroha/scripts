...
package HSDES::RuleEngine::ActionCollection;
...
sub getActionCount {
    ...
    my $numActions = @{$self->{actions}};
    return $self->{additionalActions} + $numActions;
}

########################################################################
=head2 adjustActionCount

=cut
########################################################################

sub adjustActionCount {
    my $self = shift;
    my $delta = shift;

    $self->{additionalActions} += $delta;
}
