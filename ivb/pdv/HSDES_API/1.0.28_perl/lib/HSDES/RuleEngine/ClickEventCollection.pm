...
package HSDES::RuleEngine::ClickEventCollection;
...
########################################################################
=head2 new(buttonName, alwaysAfterRules)
...
sub handler {
    my $self = shift;
    my $callback = shift;
    my $handlers = $self->{clickHandlers};
...
sub registerHandler {
    ...
    unshift @{$self->{clickHandlers}}, $handler;
}
