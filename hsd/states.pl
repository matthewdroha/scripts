...
print "$start\n";
print "$end\n";
die "Fail: Enter start and end date.\n" unless ($start and $end);
...
my $results = $queryObj->execQuery(qq(select id,rev,title,status_reason,submitted_date where submitted_date GREATER_THAN_OR_EQUAL_TO '${start}' and submitted_date LESS_THAN '${end}' and release != 'hsdes' and status IS_NOT_EMPTY and tenant = 'pds_services' and subject = 'support')) or die $queryObj->getLastErrorMessage();
