...
my %cell_table;
my @csvdata;
my @master_field_list = ('LIBRARY', 'CELL', 'CELLVIEW', 'LAST MODIFIED', 'REALPATH');
# Foreach library
foreach my $library (sort keys %targetpath_hash) {
  ...
  foreach my $cell (@cells) {
    ...
    unless (-e $targetfile) {
      $logfh->warnp("Could not find file->($targetfile)");
      next;
    }
    ...
    $cell_table{$library}{$cell}{$cellview}{'REALPATH'} = $targetfile;
    my @stat_list = stat($targetfile);
    my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime($stat_list[9]);
    $year += 1900;
    
    my $mtime = sprintf("%d-%02d-%02d %02d:%02d:%02d",
			$year, $mon, $mday, $hour, $min, $sec);
    $cell_table{$library}{$cell}{$cellview}{'LAST MODIFIED'} = $mtime;
    push(@csvdata, join(',',$library, $cell, $cellview, $mtime, $targetfile));
  }
}
